window.contacts = [
  ...
  function render(contacts) {
    ...
    cont.innerHTML = contacts.map(renderContact).join("");
    loadCities(contacts);
  }
  ...
  function filterHandler() {
    ...
    select.addEventListener("change", (event) => {
      ...
      if (selected === "0") render(contacts);
      else render(filterByCity(selected));
    });
  }
  ...
  function loadCities(contacts) {
    ...
    const uniqueCities = [...new Set(cities)].map(
    ...
    options.innerHTML = uniqueCities.join("");
  }
  ...
  function deleteButtonHandler() {
    const dlt = document.querySelectorAll(".deleteBtn");
    dlt.forEach((btn) => {
      btn.addEventListener("click", (event) => {
        const contact = event.target.parentNode;
        deleteContact(contact.dataset.id);
        main();
      });
    });
  }
